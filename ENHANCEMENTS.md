I would like to plan some changes to CPW. Here are some high-level needs to begin the planning
  process (I don't want to write any code at this time):
  - Libraries should become available once they are added to the library registry without
  restarting / rebuilding the CPW app.
  - A user (also known as a patron) can look up their library by name, using fuzzy search to
  find their library from among all those currently available from the registry's library feed.
  - A user should be able to indicate that they are affiliated with a library ("Add this
  library" functionality).
  - A user may be affiliated with more than one library.
  - When a user adds a library affiliation, they should have the option to persist that affiliation in the browser,
    so that they can be associated with that library without looking up the library again when they visit the CPW instance in
    the future from the same browser. They should receive a warning about saving this information on a public computer.
  - A user should be able to remove a persisted library affiliation.
  - When a user logs into a library, they should have the option to persist their credentials in the browser, so that they
    don't have to log in again when they visit the CPW instance in the future from the same browser. 
    They should receive a warning about saving this information on a public computer.
  - When a user with cached (current or active) or persisted library affiliations visits the base URL of a CPW instance (as opposed to URL of a specific library
  within the instance), they should be given the option of (1) visiting one of their affiliated libraries or (2) visiting the
  catalog of another library.
...
## Plan for discussion
  ...
  - Library registry support exists (src/config/fetch-config.js:47-77) but fetches at build
  time
  - APP_CONFIG is baked into the Next.js build via next.config.js (lines 34-38)
  - Static paths use fallback: true for ISR (Incremental Static Regeneration) with 1-hour
  revalidation
  ...
  3. Credential Expiration: Should persistent credentials have a shorter expiration than 30
  days?
  4. Search Metadata: Does the registry feed include enough metadata for good search
...
⏺ This implementation plan addresses all your requirements:
  ...
  ✅ Runtime library availability - Libraries become available without rebuild via runtime
  registry fetching✅ Library search - Fuzzy search component to find libraries by name✅
  Library affiliation - "Add this library" functionality with localStorage persistence✅
  Multiple affiliations - Users can be affiliated with multiple libraries✅ Affiliation
  persistence - Optional browser storage with public computer warnings✅ Affiliation
  management - Ability to remove persisted affiliations✅ Credential persistence - Optional
  "Remember me" for login credentials✅ Smart home page - Shows affiliated libraries with
  option to find more

  The plan maintains backward compatibility, leverages existing patterns (SWR, cookies, ISR),
  and provides a clear migration path. The phased approach allows incremental development and
  testing.